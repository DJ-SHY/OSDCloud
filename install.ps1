...
Reg.exe add "HKU\default\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" /v "DisableAnimations" /t REG_SZ /d "reg add \"HKCU\Control Panel\Desktop\" /v UserPreferencesMask /t REG_BINARY /d 9012078010000000 /f | Out-Null" /f | Out-Null
...
Reg.exe add "HKU\S-1-5-20\Software\Microsoft\Windows\CurrentVersion\DeliveryOptimization\Settings" /v "DownloadMode" /t REG_DWORD /d "0" /f | Out-Null
...
Reg.exe add "HKU\default\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" /v "RestoreWin10ContextMenu" /t REG_SZ /d "reg add HKCU\Software\Classes\CLSID\{86ca1aa0-34aa-4e8b-a509-50c905bae2a2}\InprocServer32 /f | Out-Null /ve" /f | Out-Null
...
Reg.exe delete "HKU\default\SOFTWARE\Microsoft\Siuf\Rules" /v "PeriodInNanoSeconds" /f | Out-Null
...
Reg.exe add "HKU\default\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" /v "SetTaskbarSearchbox" /t REG_SZ /d "reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Search /v SearchboxTaskbarMode /t REG_DWORD /d 1 /f | Out-Null" /f | Out-Null
...
Reg.exe delete "HKU\default\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "NoStartMenuMorePrograms" /f | Out-Null
Reg.exe add "HKU\default\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /f | Out-Null
